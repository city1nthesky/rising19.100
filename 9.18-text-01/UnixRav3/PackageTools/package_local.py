...
config = """{
    ...
    "platform": [
        ...
        },
        {
            "name": "RavDaemon",
            "pid": 10067,
            "bin": ["RAVGJCZ0","RAVGLTX0","RAVZDFY0", "RAVRJJH0"]
        },
        {
            "name": "RavQt_Config",
            "pid": 10069,
...
class PackageLib(object):
    ...
    def copyPackageFile(self, filepath):
        print "xxx copyPackageFile:" + filepath
        namefile = filepath
    ...
    def prepareLocalPackFiles(self, path, arch, copy_setup):
        print "prepareLocalPackFiles: " + path
        for dirpath,dirnames,filenames in os.walk(path):
            for file in filenames:
                filename = dirpath + "/" + file
                print "Find local file:" + filename
                if file == "ravsetup.bin" :
                    if copy_setup == 1:
                        self.cfs.copyFile(filename, os.path.join(self.outDir, "setup", arch, "ravsetup.bin"))
                else:
                    self.copyPackageFile(filename)
    ...
    def pack_local(self, version, build_path, arch=None, virusdb=None):
        assert self.outDir
        assert self.packDir
        assert self.packTools
        ...
        self.refreshPackDir()
        #self.preparePackFiles(arch)
        self.copyPackageFile(virusdb)
         
        self.refreshOutDir()
        self.prepareLocalPackFiles(build_path, arch, 0)
        state, commpck = self.packCommon()
        if state:
            print "pack:created package file " + commpck 
        else:
    ...
    def generateLocalProductConfigFile(self, version):
        timestr = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
        f = open(os.path.join(self.outDir, "product.xml"), "w")
        f.write("<rpklist version=\"%s\" virver=\"%s\" updatetime=\"%s\">\n" % (version, self.local_vdb_ver, timestr))
        f.write("<item name=\"ep\" version=\"%s\" md5=\"%s\" size=\"%d\" updatetime=\"%s\" server=\"0\" client=\"1\" mustinstall=\"1\"/>\n" % \
                    (version, self.ep_md5, self.ep_size, timestr))
        f.write("<item name=\"xav\" version=\"%s\" md5=\"%s\" size=\"%d\" updatetime=\"%s\" server=\"0\" client=\"0\" mustinstall=\"0\"/>\n" % \
                    (version, self.xav_md5, self.xav_size, timestr))
...
def build_package_from_local(version, vdb_version, build_path, folder, arch, packtool,virusdb):
    major_folder = folder
    pack = PackageLib(config, os.path.join(major_folder, arch, "packet.dir"), os.path.join(major_folder, "smb250"))
    pack.local_vdb_ver = vdb_version
    pack.setOutDir(os.path.join(major_folder, "package", version))
    cmfolder = os.path.join(major_folder, "package")
    ...
    pack.packTools = packtool
    pack.packDir = os.path.join(folder, arch, "packet.dir")
    pack.pack_local(version, build_path, arch, virusdb)
    pack.prepareLocalPackFiles(build_path, arch, 1)
    pack.generateLocalProductConfigFile(version)
    pack.cfs.copyFile(os.path.join(pack.packDir, "setup.sh"), os.path.join(pack.outDir, "setup.sh"))
    print "pack:tarFile " + cmfolder + " -> " + os.path.join(cmfolder, "RAV_%s.tar.gz"%(version))
    pack.tarFile(cmfolder, os.path.join(cmfolder, "RAV_%s.tar.gz"%(version)))
    shutil.move(os.path.join(cmfolder, "RAV_%s.tar.gz"%(version)), os.path.join(major_folder, "RAV_{0}_{1}.tar.gz".format(version, arch)))
    print "move result file to " + os.path.join(major_folder, "RAV_{0}_{1}.tar.gz".format(version, arch))
    #shutil.rmtree(pack.outDir)
    #shutil.rmtree(cmfolder)
    #shutil.rmtree(pack.packDir)
...
def main(argv):
    ...
    virver  = ''
    try:
        opts, args = getopt.getopt(argv,"hi:o:v:a:t:d:z:",["input=","ofile=","version=","arch=","tool=","virsudb=", "virver="])
    except getopt.GetoptError:
        print 'package.py -i <build folder> -o <outputfile> -v <version> -a <arch> -t <toolpath> -d <virusdb path> -z <virver>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print "这是一个打本地包的程序，运行环境为python2.7，首先将resources里的文件拷贝至编译目录的plugins目录里"
            print "其次将第三方工程的SO和杀毒引擎的SO拷贝至LIB目录"
            print 'package.py -i <build folder> -o <outputfile> -v <version> -a <arch> -t <toolpath> -d <virusdb path> -z <virver>'
            sys.exit()
            ...
            toolfile = arg
        elif opt in("-d", "--virusdb"):
            virusdb = arg
        elif opt in("-z", "--virver"):
            virver = arg

    """
    main_folder = "/home/rising/UnixRavWorkDir"
    folder = os.path.join(main_folder, "make_package")
    arch = execCmd("uname -m")
    if arch[len(arch)-1] == '\n':
        arch = arch[0:len(arch)-1]
    build_path = os.path.join(main_folder, "build/3.0.0.1")
    packtool = os.path.join(main_folder, "build/3.0.0.1/tools/PackTools")
    """
  
    folder = '/tmp/rspack'
    workdir = os.path.join(folder, arch, "packet.dir")
    if os.path.exists(workdir):
        shutil.rmtree(workdir)
    os.makedirs(workdir)

    build_package_from_local(version, virver, infolder, folder, arch, toolfile,virusdb)
 
    out = "RAV_%s_%s.tar.gz"%(version, arch)
    if os.path.exists(os.path.join(folder, out)):
        shutil.move(os.path.join(folder, out), outfile)
...
if __name__ == "__main__":
    try:
        ...
        print "error, please package.py -h get help info" 
