 ...
 */
JDiff::JDiff(
    JFileIn * const apFilOrg, JFileIn * const apFilNew,
    JOut * const apOut,
...
int JDiff::jdiff()
{
    ...
    offset_t lzEql = 0;			/* accumulated equal bytes */
	...
	/* Return code */
	// IT'S ERROR!
	if (lcNew < IO_EOB || lcOrg < IO_EOB)
	{
		return (lcNew < lcOrg)?lcNew:lcOrg;
	}

	// FULL-EQUL
	if( lzPosOrg == lzPosNew && bFullSame && lcNew == IO_EOF && lcOrg == IO_EOF )
		return J_SAME;
 ...
 * Flush output
 */
void JDiff::ufPutEql(const offset_t &lzPosOrg, const offset_t &lzPosNew, offset_t &lzEql, bool &lbEql){
    /* Output accumulated equals */
    if (lzEql > 0){
 ...
 */
int JDiff::ufFndAhd (
  offset_t const &azRedOrg,
  offset_t const &azRedNew,
  offset_t &azSkpOrg,
  offset_t &azSkpNew,
  offset_t &azAhd
)
{ offset_t lzFndOrg=0;   /* Found position within original file                 */
  offset_t lzFndNew=0;   /* Found position within new file                      */
  offset_t lzBseOrg;     /* Base position on original file: gbSrcBkt?0:alRedOrg */
   ...
   */
  offset_t liMax; /* Max number of bytes to read */
  if (miSrcScn == 2){
   ...
   */
  offset_t liBck; /* Number of bytes to look back */
  ...
  if (gpMch->cleanup(azRedNew - gpHsh->get_reliability())){
      ...
      if (miSrcScn > 0) miValOrg = IO_EOB ;

      /* Scroll through both files until an equal hash value has been found */
      while ((liMax > 0) && ((miValNew > IO_EOF ) || (miValOrg > IO_EOF))) {
   ...
   * Check for errors
   */
  if (miValNew < IO_EOB || miValOrg < IO_EOB){
 ...
 */
void JDiff::ufFndAhdGet(JFileIn *apFil, const offset_t &azPos, int &acVal, int &aiEql, int aiSft)
{
  int lcPrv = acVal ;
...
int JDiff::ufFndAhdScn ()
{
  ...
  offset_t lzPosOrg=0;     // Position within original file
...
int Diff( JFileIn& apFilOrg, JFileIn& apFilNew, JFileBOut& apOut, 
		 ...
		 const bool abCmpAll )
{
	JOutBin BinOut( apOut );
	JDiff oDiff( &apFilOrg, &apFilNew, &BinOut, aiHshSze, 0, abSrcBkt, aiSrcScn, aiMchMax, aiMchMin, aiAhdMax, abCmpAll );
	return oDiff.jdiff();
}
