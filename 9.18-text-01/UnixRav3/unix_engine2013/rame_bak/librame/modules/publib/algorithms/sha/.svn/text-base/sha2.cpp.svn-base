...
#	if TARGET_ENDIAN==BYTE_ENDIAN_L
		#define PLATFORM_BYTE_ORDER SHA_LITTLE_ENDIAN	
#	else
#		define PLATFORM_BYTE_ORDER SHA_BIG_ENDIAN	
#	endif
...
#if !defined(PLATFORM_BYTE_ORDER)
#  error Please set undetermined byte order (lines 180 or 182 of sha2.cpp).
#endif
...
#define h2(i) m_ctx.wbuf[i & 15] += \
    g256_1(m_ctx.wbuf[(i + 14) & 15]) + m_ctx.wbuf[(i + 9) & 15] + g256_0(m_ctx.wbuf[(i + 1) & 15])

#define h2_cycle(i,j)  \
    v[(7 - i) & 7] += (j ? h2(i) : m_ctx.wbuf[i & 15]) + k256[i + j] \
...
/* SHA256 mixing data */
const SHA2_256::sha2_32t SHA2_256::k256[64] =
{
...
void SHA2_256::compile()
{
	sha2_32t v[8], j;

	memcpy(v, m_ctx.hash, 8 * sizeof(sha2_32t));
	...
	m_ctx.hash[0] += v[0]; m_ctx.hash[1] += v[1]; m_ctx.hash[2] += v[2]; m_ctx.hash[3] += v[3];
	m_ctx.hash[4] += v[4]; m_ctx.hash[5] += v[5]; m_ctx.hash[6] += v[6]; m_ctx.hash[7] += v[7];
}


/* SHA256 Final padding and digest calculation */
const SHA2_256::sha2_32t SHA2_256::m1[4] =
{
	n_u32(00000000), n_u32(ff000000), n_u32(ffff0000), n_u32(ffffff00)
};
const SHA2_256::sha2_32t SHA2_256::b1[4] =
{
...
void SHA2_256::Final()
{
	sha2_32t i = (sha2_32t)(m_ctx.count[0] & SHA256_MASK);

	bsw_32(m_ctx.wbuf, (i + 3) >> 2)
	...
	m_ctx.wbuf[i >> 2] = (m_ctx.wbuf[i >> 2] & m1[i & 3]) | b1[i & 3];
    ...
    /* word values.                                                 */
    m_ctx.wbuf[14] = (m_ctx.count[1] << 3) | (m_ctx.count[0] >> 29);
    m_ctx.wbuf[15] = m_ctx.count[0] << 3;
    ...
    for(i = 0; i < SHA256_DIGEST_SIZE; ++i)
        m_digest[i] = (unsigned char)(m_ctx.hash[i >> 2] >> 8 * (~i & 3));

	// fixed byte order by zhouhui 2006-12-12
	sha2_32t *p = (sha2_32t*)&m_digest[0];
	bsw_32(p, (SHA256_DIGEST_SIZE) >> 2)
}


/* SHA256 hash data in an array of bytes into hash buffer   */
/* and call the hash_compile function as required.          */
void SHA2_256::Update(const void *pvSrc, size_t cbSize)
{
	const unsigned char *data = (unsigned char*)pvSrc;
	size_t len = cbSize;

	sha2_32t pos = (sha2_32t)(m_ctx.count[0] & SHA256_MASK), 
			 space = SHA256_BLOCK_SIZE - pos;
	const unsigned char *sp = data;

	if((m_ctx.count[0] += len) < len)
		++(m_ctx.count[1]);

	while(len >= space) /* tranfer whole blocks while possible  */
	{
		memcpy(((unsigned char*)m_ctx.wbuf) + pos, sp, space);
		sp += space; len -= space; space = SHA256_BLOCK_SIZE; pos = 0; 
		bsw_32(m_ctx.wbuf, SHA256_BLOCK_SIZE >> 2)
		compile();
	}

	memcpy(((unsigned char*)m_ctx.wbuf) + pos, sp, len);
}


/* SHA256 initialisation data */
const SHA2_256::sha2_32t SHA2_256::i256[8] =
{
...
void SHA2_256::Init()
{
	...
	memcpy(m_ctx.hash, i256, 8 * sizeof(sha2_32t));
}
...
/* FIPS-180, different variables are 'rotated' on each round, returning     */
/* to their starting positions every eight rounds                           */
#define h5(i) m_ctx.wbuf[i & 15] += \
	g512_1(m_ctx.wbuf[(i + 14) & 15]) + m_ctx.wbuf[(i + 9) & 15] + g512_0(m_ctx.wbuf[(i + 1) & 15])

#define h5_cycle(i,j)  \
	v[(7 - i) & 7] += (j ? h5(i) : m_ctx.wbuf[i & 15]) + k512[i + j] \
		+ s512_1(v[(4 - i) & 7]) + ch(v[(4 - i) & 7], v[(5 - i) & 7], v[(6 - i) & 7]); \
	v[(3 - i) & 7] += v[(7 - i) & 7]; \
	v[(7 - i) & 7] += s512_0(v[(0 - i) & 7]) + maj(v[(0 - i) & 7], v[(1 - i) & 7], v[(2 - i) & 7])

/* SHA384/SHA512 mixing data */
const SHA2_512::sha2_64t SHA2_512::k512[80] =
{
...
void SHA2_512::compile()
{
	sha2_64t v[8];
	sha2_32t j;

	memcpy(v, m_ctx.hash, 8 * sizeof(sha2_64t));
...
const SHA2_512::sha2_64t SHA2_512::m2[8] =
{
...
const SHA2_512::sha2_64t SHA2_512::b2[8] =
{
...
void SHA2_512::end(unsigned int hlen)
{
	sha2_32t i = (sha2_32t)(m_ctx.count[0] & SHA512_MASK);

	bsw_64(m_ctx.wbuf, (i + 7) >> 3);
	...
	/* we now need to mask valid bytes and add the padding which is */
	/* a single 1 bit and as many zero bits as necessary.           */
	m_ctx.wbuf[i >> 3] = (m_ctx.wbuf[i >> 3] & m2[i & 7]) | b2[i & 7];
    ...
    /* word values.                                                 */
	m_ctx.wbuf[14] = (m_ctx.count[1] << 3) | (m_ctx.count[0] >> 61);
	m_ctx.wbuf[15] = m_ctx.count[0] << 3;
	...
	for(i = 0; i < hlen; ++i)
		m_digest[i] = (unsigned char)(m_ctx.hash[i >> 3] >> 8 * (~i & 7));
...
/* buffer will now go to the high end of words on BOTH big  */
/* and little endian systems                                */
void SHA2_512::Update(const void *pvSrc, size_t cbSize)
{
	const unsigned char *data = (unsigned char*)pvSrc;
	size_t len = cbSize;

	sha2_32t pos = (sha2_32t)(m_ctx.count[0] & SHA512_MASK), 
	...
	while(len >= space)	 /* tranfer whole blocks while possible  */
	{
		...
		bsw_64(m_ctx.wbuf, SHA512_BLOCK_SIZE >> 3);		
		compile();
	}
...
const SHA2_512::sha2_64t SHA2_512::i512[80] =
{
...
{
	m_ctx.count[0] = m_ctx.count[1] = 0;
	memcpy(m_ctx.hash, i512, 8 * sizeof(sha2_64t));
}
...
const SHA2_384::sha2_64t SHA2_384::i384[80] = 
{
...
{
	m_ctx.count[0] = m_ctx.count[1] = 0;
	memcpy(m_ctx.hash, i384, 8 * sizeof(sha2_64t));
}
