...
#  define aligned_uint_8t(n)  __declspec(align(n)) uint_8t _al_##n
#elif defined( __GNUC__ )
#  define aligned_uint_8t(n)  uint_8t _al_##n __attribute__ ((aligned(n)))
#endif

aligned_uint_8t( 4);        /* a variable that is 4 byte aligned  */
aligned_uint_8t(16);        /* a variable that is 16 byte aligned */

#define addr_offset(x,n)    (((uint_8t*)(x) - &_al_##n) & ((n) - 1))
...
aligned_array(ULONG,    enc_gen_table, 12, 16) =    NEH_ENC_GEN_DATA;
aligned_array(ULONG,   enc_load_table, 12, 16) =   NEH_ENC_LOAD_DATA;
aligned_array(ULONG, enc_hybrid_table, 12, 16) = NEH_ENC_HYBRID_DATA;
aligned_array(ULONG,    dec_gen_table, 12, 16) =    NEH_DEC_GEN_DATA;
aligned_array(ULONG,   dec_load_table, 12, 16) =   NEH_DEC_LOAD_DATA;
aligned_array(ULONG, dec_hybrid_table, 12, 16) = NEH_DEC_HYBRID_DATA;
...
#define via_cwd(cwd, ty, dir, len) \
    ULONG* cwd = (dir##_##ty##_table + ((len - 128) >> 4))

#else

#define via_cwd(cwd, ty, dir, len)              \
    aligned_auto(ULONG, cwd, 4, 16);    \
    cwd[1] = cwd[2] = cwd[3] = 0;               \
