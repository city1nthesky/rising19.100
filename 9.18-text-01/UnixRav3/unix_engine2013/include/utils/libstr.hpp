...
namespace tcs
{
	...
	// strlwr new
	template < class XCHAR >
	XCHAR* lwr( XCHAR * p, int l)
	{
		for( XCHAR * p1 = p; *p1 && p1 < p + l; ++ p1 ) *p1 = ::tolower(*p1);
		return p;
	}
	...
	// strupr new
	template < class XCHAR >
	XCHAR* upr( XCHAR * p, int l )
	{
		for( XCHAR * p1 = p; *p1 && p1 < p + l; ++ p1 ) *p1 = ::toupper(*p1);
		return p;
	}
	...
	template < class XCHAR >
	XCHAR* istr( const XCHAR* str1, const XCHAR* str2 )
	{
		XCHAR *cp = (XCHAR *) str1;
		XCHAR *s1, *s2;
	...
	inline size_t otoa(char *mbstr, const OLECHAR *ocstr, size_t count)
	{
		return wcstombs(mbstr, ocstr, count)+1;
	}

	inline size_t atoo(OLECHAR *ocstr, const char *mbstr, size_t count)
	{
		return mbstowcs(ocstr, mbstr, count)+1;
	}
	...
	inline size_t otoa(char *mbstr, const OLECHAR *ocstr, size_t count)
	{
		const OLECHAR *wp = ocstr;
		while(wp < ocstr + count && *wp) *mbstr++ = *wp++;
		return (wp - ocstr) / 2 + 1;
	}

	inline size_t atoo(OLECHAR *ocstr, const char *mbstr, size_t count)
	{
		const char *mbp = mbstr;
		while(mbp < mbstr + count && *mbp) *ocstr++ = *mbp++;
		return mbp - mbstr + 1;
	}
...
//////////////////////////////////////////////////////////////////////////
	...
	#define rs_wcsdup		wcsdup
	#define rs_strlen		strlen
	#define rs_wcslen		wcslen
	#define rs_strlwr		strlwr
	#define rs_strupr		strupr
	#define rs_wcsupr		wcsupr
	#define rs_strchr		strchr
	#define rs_wcschr		wcschr
	#define rs_strstr		strstr
	#define rs_wcsstr		wcsstr
	#define rs_strcmp		strcmp
	#define rs_wcscmp		wcscmp
	#define rs_strncmp		strncmp
	#define rs_wcsncmp		wcsncmp
	#define rs_strcpy		strcpy
	#define rs_wcscpy		wcscpy
	#define rs_strncpy		strncpy
	#define rs_wcsncpy		wcsncpy
	#define rs_strcat		strcat
	#define rs_wcscat		wcscat
	#define rs_strncat		strncat
	#define rs_wcsncat		wcsncat
	#define rs_strrchr		strrchr
	#define rs_wcsrchr		wcsrchr
	#if (TARGET_OS == OS_WINDOWS)
	#define rs_wcslwr		wcslwr
	#define rs_stricmp		stricmp
	#define rs_wcsicmp		wcsicmp
	#define rs_strnicmp		_strnicmp
	#define rs_wcsnicmp		_wcsnicmp
	#else
	#define rs_wcslwr		tcslwr
	#define rs_stricmp		strcasecmp
	#define rs_wcsicmp		wcscasecmp
	#define rs_strnicmp		strncasecmp
	#define rs_wcsnicmp		wcsncasecmp
	#endif
#else
	#define rs_strdup		tcsdup
	#define rs_wcsdup		tcsdup
	#define rs_strlen		tcslen
	#define rs_wcslen		tcslen
	#define rs_strlwr		tcslwr
	#define rs_wcslwr		tcslwr
	#define rs_strupr		tcsupr
	#define rs_wcsupr		tcsupr
	#define rs_strchr		tcschr
	#define rs_wcschr		tcschr
	#define rs_strstr		tcsstr
	#define rs_wcsstr		tcsstr
	#define rs_strcmp		tcscmp
	#define rs_wcscmp		tcscmp
	#define rs_strncmp		tcsncmp
	#define rs_wcsncmp		tcsncmp
	#define rs_strcpy		tcscpy
	#define rs_wcscpy		tcscpy
	#define rs_strncpy		tcsncpy
	#define rs_wcsncpy		tcsncpy
	#define rs_strcat		tcscat
	#define rs_wcscat		tcscat
	#define rs_strncat		tcsncat
	#define rs_wcsncat		tcsncat
	#define rs_strrchr		tcsrchr
	#define rs_wcsrchr		tcsrchr
	#define rs_stricmp		tcsicmp
	#define rs_wcsicmp		tcsicmp
	#define rs_strnicmp		tcsnicmp
	#define rs_wcsnicmp		tcsnicmp
#endif
