...
#define _xvcall_		
	...
	struct xvalue_t : Value
	{
		...
		_xvcall_ xvalue_t( IUnknown * punk )
		{
			ValueInit( this );
			if( !set(punk) ) throw ExcepValueInit;
		}
		...
		//////////////////////////////////////////////////////////////////////////
		_xvcall_ uint8_t get_type() const {
			return _type.type;
		}
		...
		_xvcall_ xv::xvalue_t& operator [] ( const std::string name )
		{
		...
		_xvcall_ xv::xvalue_t& operator [] ( int index )
		{
			return ref( index );
		}
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dInt ) ) 
				return false;
			i = (int8_t)integer;
			return true;
		}
		...
		{
			if( ValueCheckType( XVALUE_CONVERT this, dReal ) )
			{
				i = real;
			}
			else if( ValueCheckType( XVALUE_CONVERT this, dInt ) ) 
			{
				i = (double)integer;
			}
			else
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dBuffer ) ) 
				return false;
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dUniStr ) ) 
				return false;
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dLocalStr ) ) 
				return false;
			refp<vStringA> ostr( complex );
			mbstr = ostr->c_str();
			return true;
		}
		_xvcall_ bool get( std::string & mbstr ) const
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dLocalStr ) ) 
				return false;
			refp<vStringA> ostr( complex );
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dUnknown ) ) 
				return false;
			p = punk;
			return true;
		}
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dLocalStr ) &&
				!ValueCheckType( XVALUE_CONVERT this, dUtf8Str ) )
				return false;
			refp<vStringA> ostr( complex );
		...
		{
			return ValueSet( XVALUE_CONVERT this, &rval ) != FALSE;
		}
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dObject ) ) 
				return false;
		...
		_xvcall_ bool exists( std::string name ) 
		{
		...
		_xvcall_ bool exists( const char * name ) 
		{
			if( !isObject() ) return false;
			xv::xvalue_t tmp;
			return get( name, tmp );
		...
		template < class T >
		_xvcall_ bool get( std::string name, T & elmt ) const
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dObject ) ) 
				return false;
			refp<vObject> obj = complex;
			xvalue_t xo;
			if( !obj->get( name.c_str(), xo ) )
				return false;
			return xo.get( elmt );
		}
		template < class T >
		...
		{
			if( !ValueCheckType( XVALUE_CONVERT this, dArray ) ) 
				return false;
		...
		_xvcall_ xv::xvalue_t& ref( const char * name ) 
		{
			if( ValueCheckType( XVALUE_CONVERT this, dNull ) )
			{
				if( !ValueSetType( XVALUE_CONVERT this, dObject ) )
					throw ExcepValueInit;
			}
			else if( !ValueCheckType( XVALUE_CONVERT this, dObject ) ) 
			{
		...
		_xvcall_ xv::xvalue_t& ref( size_t index ) 
		{
			if( ValueCheckType( XVALUE_CONVERT this, dNull ) )
			{
				if( !ValueSetType( XVALUE_CONVERT this, dArray ) )
					throw ExcepValueInit;
			}
			else if( !ValueCheckType( XVALUE_CONVERT this, dArray ) ) 
			{
				throw ExcepTypeConvert;
			}
			refp<vArray> arry = complex;
		...
		_xvcall_ bool set( std::string & lstr )
		{
			if( !ValueSetType( this, dLocalStr ) ) 
				return false;
			refp<vStringA> mbs(complex);
			return mbs->assign( lstr.c_str() );
		}
		_xvcall_ bool set( const std::string & lstr )
		{
			if( !ValueSetType( this, dLocalStr ) ) 
				return false;
			refp<vStringA> mbs(complex);
		...
		{
			return ValueSet( XVALUE_CONVERT &val, this ) != FALSE;
		}
		...
		_xvcall_ int64_t asInt() {
			return integer;
		}
		_xvcall_ uint64_t asUInt() {
			return (uint64_t)integer;
		}
		_xvcall_ const char * asCString() {
			return ((vStringA*)complex)->c_str();
		}
		_xvcall_ double asDouble() {
			return real;
		}
		_xvcall_ bool asBool() {
			return vbool;
		}
		...
		//////////////////////////////////////////////////////////////////////////
		_xvcall_ chunk_t to_chunk()
		{
			chunk_t ret;
			if( !IsBuffer() ) return ret;
			ret.Data = asBuffer(false)->base();
			ret.Length = asBuffer(false)->length();
			return ret;
		}
		//////////////////////////////////////////////////////////////////////////
		...
		_xvcall_ xvalue_t & operator = ( std::string & cstr ) 
		{
			if( set( cstr ) ) return *this;
			throw ExcepValueInit;
		}
		...
		_xvcall_ bool load_json( const void * bp, size_t cb )
		{
			if( !ValueClear( this ) ) return false;
			return ValueFromJson(this , (LPCSTR)bp , cb) ? true : false;
		}
		...
		struct txtstm : Output
		{
			std::string	_text;
			_xvcall_ bool write( const void * data, size_t bytes )
			{
				const char * tmp = (const char *)data;
				_text.append( tmp, tmp + bytes );
				return true;
			}
			_xvcall_ const std::string get() 
			{
				return _text;
			}
		};
		...
		_xvcall_ bool IsBoolean()
		{
			return (_type.type == dBoolean) ;
		}
		_xvcall_ bool IsBuffer()
		{
			return (_type.type == dBuffer) ;
		}
		_xvcall_ Value detach()
		{
			Value tmp = *this;
			_type.type = dNull;
			_type.bsiz = 0;
		...
		_xvcall_ void CopyTo( Value & r )
		{
			((xvalue_t&)r) = *this;
		}
		_xvcall_ size_t length() 
		{
			if( _type.type == dInt )
			{
				return 1 << _type.bsiz;
			}
			if( _type.type == dBoolean )
			{
				return sizeof(vbool);
			}
			if( _type.type == dReal )
			...
			if( _type.type == dLocalStr || _type.type == dUtf8Str || _type.type == dUniStr ||
				_type.type == dArray || _type.type == dObject || _type.type == dBuffer )
			{
				if( !complex ) return false;
		...
		template < class T >
		_xvcall_ T * buffer_cast() 
		{
			refp<vBuffer> buf = asBuffer(false);
			if( !buf ) return NULL;
			if( buf->length() < sizeof(T) ) return NULL;
			return (T*)buf->base();
		}
		...
		_xvcall_ I& ascom()
		{
			if( !isUnknownPtr() ) throw ExcepTypeConvert;
			if( !punk ) throw ExcepTypeConvert;
			void * pv;
			if( FAILED( punk->QueryInterface( re_uuidof(I), &pv ) ) )
				throw ExcepTypeConvert;
			punk->Release();
			punk = (I*)pv;
			return *(I*)pv;
