...
struct com_ptr : sentry<I*, co_interface_sentry>
{
	...
	void attach( I * ip ) { base::dispose(); m_p = ip; }

	com_ptr( TCLSIDREF tClsid, IUnknown * outer = 0, IUnknown * xt = 0 ) { RoCreateInstanceNCT( tClsid, outer, re_uuidof(I), base::vpp(), xt ); };
