...
static char g_extra_folder[1024];
...
static WRes MyCreateDir(const UInt16 *name)
{
  ...
  #else
  snprintf(g_temp_buf, 512, "%s/%s", g_extra_folder, (const char *)buf.data);
  mkdir(g_temp_buf, 0777)
  #endif
  == 0 ? 0 : errno;
...
int MY_CDECL main_func(int numargs, char *args[])
{
  ISzAlloc allocImp;
...
int decompress_7z(const char *zip, const char *outfolder) {
    int num = 3;
    const char *params[3] = {NULL, "x", zip};

    strcpy(g_extra_folder, outfolder);

    return main_func(3, (char**)params);
}
