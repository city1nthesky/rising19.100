...
LONG RsUpgradeEncrypt(LPTSTR pOut, LONG lOutBufSize, LPCTSTR pUserID, LPCTSTR pSn, DWORD dwTimeSetup, DWORD dwTimeOver, LONG lType, LONG lValidate)
{
    ...
    sprintf(szSetupTime, _T("%ld"), dwTimeSetup);
    sprintf(szOverTime, _T("%ld"), dwTimeOver);
    sprintf(szType, _T("%ld"), lType);
    sprintf(szValidate, _T("%ld"), lValidate);
...
LONG RsUpgradeEncrypt(LPTSTR pOut, LONG lOutBufSize, LPCTSTR pIn)
{
    ...
    LONG lNeedLength = 7 + _tcslen(pIn);	// 加解密随机数在开始6个字符, 最后一个字节是校验码
    ...
    int nRandomIndex = 0;
    for(i = 6; i < lBeforeBase64Length; i++)
    {
    ...
    LONG lOutLength = base64_encode((BYTE*)szTemp, lBeforeBase64Length, pOut);
...
{
    LONG lOutNeedSize = _tcslen(pIn) * 3 / 4 - 6;
    if(lOutNeedSize < 1)
        return 0;
    ...
    //BASE64
    LONG lBase64After = base64_decode(pIn, (BYTE*)szTemp);
...
#ifdef TEST_EXAMPLE
//
//gcc -DTEST_EXAMPLE -o rsencrypt RsUpgradeCode.cpp Base64.cpp
//
//standard input code and encrypt code
/*
RsUpgradeEncrypt( out, out_size, "4AKT8VVJIA42", "F84WTW-SR17W7-VVQHS5-2P5200",0, 0, 9, 2025482);
*/
int main( int argc, char **argv)
{
    char ecode[2048+1], dcode[2048+1];
    long ret = 1;

    //test for RsUpgradeEncrypt and RsUpgradeDecrypt
    bzero( ecode, 2049 );
    ret = RsUpgradeEncrypt( ecode, 2048, "4AKT8VVJIA42", "F84WTW-SR17W7-VVQHS5-2P5200",0, 0, 9, 2025482);
    if( ret <= 0 ){
        printf(" RsUpdateEncrypt fail with error code  %d\n", ret );
        return -1;
    }

    bzero( dcode, 2049 );
    ret = RsUpgradeDecrypt( dcode, 2048, ecode );
    if( ret <= 0 ){
        printf(" RsUpgradeDecrypt fail with error code %d\n", ret );
        return -3;
    }
    printf("Decrypt code:\"%s\"\n", dcode);

    //test for RsUpgradeEnStr and RsUpgradeDecrypt
    sprintf(dcode,"A FOX JUMP THE Z");
    memset( ecode, 0, 2048 );
    ret = RsUpgradeEncrypt( ecode, 2048, dcode);
    if( ret <= 0 ){
        printf("RsUpgradeEnstr fail with erro code %d\n", ret );
        return -4;
    }
    bzero( dcode, 2049 );
    ret = RsUpgradeDecrypt( dcode, 2048, ecode );
    ...
    printf("Decrypt str code:\"%s\"\n", dcode);


    //decrypt command line input
    if( argc > 1 )
    {
        memset(ecode, 0, 2048 );
        strcpy( ecode, argv[1]);
