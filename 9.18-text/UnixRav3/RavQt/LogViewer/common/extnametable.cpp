    ...
    :QTableView(parent)
{
    ...
    m_pModel->item(0, 0)->setForeground(QBrush(QColor(0, 0, 0)));
    m_pModel->item(1, 0)->setForeground(QBrush(QColor(0, 0, 0)));
...
void ExtNameTable::UpdateRow(int row)
{
    ...
    for (int i = 1; i >= 0; i--)
    {
        ...
        m_pModel->item(row, i)->setForeground(QBrush(QColor(0, 0, 0)));
    }
    m_nCurrentRow = row;
}
...
void ExtNameTable::ShowToolTip(int row)
{
    Q_UNUSED(row);
...
void ExtNameTable::leaveEvent (QEvent * event)
{
    Q_UNUSED(event);
