...
struct valArray : vArray, Refalbe
{
	...
	typedef std::vector<xvalue_t> ValueVector;
	ValueVector		_elements;
	xvalue_t		_xxx;

	virtual xv::Value& ref( size_t idx )
	{
		if( idx > 10000 ) return _xxx;
		if( idx >= _elements.size() ) 
			_elements.resize(idx+1);
	...
	virtual bool 	enum_it( _enum_cb_ptr fcb, void * context )
	{
		...
		for( size_t i = 0; i < c; ++ i ) {
			if( !fcb( i, _elements[i], i + 1 >= c , context ) )
				return false;
		}
		return true;
	}
	...
	virtual bool pop( xv::Value & item ) 
	{
		if( _elements.empty() ) 
			return false;
		xvalue_t r = _elements.back();
		item = r.detach();
		_elements.pop_back();
		return true;
	}
	virtual bool resize( size_t c )
	{
		_elements.resize(c);
		return true;
	}
	virtual bool concat( xv::Value & item )
	{
		xvalue_t & x = (xvalue_t &)item;
		refp<vArray> p = x.asArray();
		if( !p ) return false;
		size_t c = p->length();
		_elements.resize( _elements.size() + c );
		for( size_t i = 0; i < c; ++ i ) {
			xvalue_t tmp;
			p->get(i, tmp);
	...
	virtual bool cloneTo( vComplex * po )
	{
		...
		if( !p->clear() ) return false;
		size_t c = _elements.size();
		if( !p->resize(c) ) return false;
		for( size_t i = 0; i < c; ++ i ) {
			if( p->set(i, _elements[i]) )
				return false;
		}
		return true;
	}
...
public:
	...
	bool clear()
	{
		...
		if( _std_str ) { delete [] _std_str; _std_str = 0; }
		_str_ptr = _mini_str;
		_length = 0;
		return true;
	}
...
BOOL WINAPI ValueClear( xv::Value * pv )
{
	...
	{
	case dNull:	case dInt: case dReal: case dBoolean: break; 
...
BOOL WINAPI ValueSetType( xv::Value* pv, xv::value_type vt )
{
	...
	case dBoolean:	pv->vbool = false;	break;
...
BOOL WINAPI ValueClone( const xv::Value* vpIn, xv::Value* vpOut )
{
	...
	case dBoolean:	vpOut->vbool = vpIn->vbool;	break;
...
BOOL WINAPI ValueSet( RS_CONST_DEF xv::Value* vpIn, xv::Value* vpOut )
{
	if( !vpIn || !vpOut ) 
		return FALSE;
	...
	case dBoolean:	vpOut->vbool = vpIn->vbool;	return TRUE;
	case dLocalStr:case dUtf8Str:case dUniStr:
	case dArray:case dObject:case dBuffer:
...
BOOL WINAPI ValueCheckType( RS_CONST_DEF xv::Value * vp, xv::value_type vt )
{
	if( !vp ) return FALSE;
...
BOOL WINAPI ValueFromJson( xv::Value* vp, LPCSTR json_string , SIZE_T len)
{
	Jxon::Reader r;
	LPCSTR first = json_string;
	if( len == -1 ) len = strlen(json_string);
	LPCSTR last = first + len;
	xv::xvalue_t* xv =  (xv::xvalue_t*)vp;
	if( !r.parse( first, last, *xv ) ) return false;
	return true;
}
...
BOOL WINAPI ValueToJson( xv::Value* vp, xv::Output * wo )
{
	if( !vp || !wo ) return FALSE;
	xvalue_t tmp( *vp );
	return Jxon::Writer::write( tmp, wo );
}
