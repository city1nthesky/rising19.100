...
#define ALERT(x)		//MessageBoxA( 0, #x, "alert", 0 );
	...
	static FORCEINLINE HRESULT SetPhysicalFileSize(IREStream* pstm , uint64_t size)
	{
		UTIL::com_ptr<IREProperty> prop(pstm);
		RASSERT( prop, E_NOINTERFACE );
		PROPVARIANT var;
		var.vt = VT_UI8;
		var.ulVal = size;
		return prop->SetProperty( PID_PHYSICAL_FILE_SIZE, &var );
	}

	static FORCEINLINE HRESULT GetPhysicalFileSize(IREStream* pstm , uint64_t& size)
	{
		UTIL::com_ptr<IREProperty> prop(pstm);
		RASSERT( prop, E_NOINTERFACE );
		const PROPVARIANT * pvar = 0 ;//
		RFAILED(prop->GetProperty( PID_PHYSICAL_FILE_SIZE, &pvar ));
		RASSERT(pvar->vt == VT_UI8 , E_FAIL);
		size = pvar->ulVal;
		return S_OK;
	}
	...
	static FORCEINLINE HRESULT MarkMailChildFile(IREStream* pstm)
	{
		UTIL::com_ptr<IREPropertyEx> prop(pstm);
		RASSERT(prop , E_INVALIDARG);
		return prop->SetProperty(PID_EMAIL_CHILD_FILE , 0);
	}
	static FORCEINLINE BOOL IsMailChildFile(IREStream* pstm)
	{
		UTIL::com_ptr<IREPropertyEx> prop(pstm);
		RASSERT(prop , E_INVALIDARG);
		return SUCCEEDED(prop->GetProperty(PID_EMAIL_CHILD_FILE, 0 ));
	}
