...
namespace nam
{
	enum MAIN_CLASS
	{
		...
		kUnwanted,
		kMonetizer,				//	套现
		kDeceptor, 			// 蒙骗, 2017/1/9, AppEsteem
	...
	static inline MAIN_CLASS _NAME2MCLS( LPCSTR name )
	{
		for( size_t i = 0; i < sizeof(_STD13_MAINCLS_NAMETABLE)/sizeof(_STD13_MAINCLS_NAMETABLE[0]); ++ i )
		{
			if( rs_stricmp( name, _STD13_MAINCLS_NAMETABLE[i].p ) )
				continue;
	...
	static inline MAIN_CLASS _NAME2MCLS2( LPCSTR name, SIZE_T cb )
	{
		for( size_t i = 0; i < sizeof(_STD13_MAINCLS_NAMETABLE)/sizeof(_STD13_MAINCLS_NAMETABLE[0]); ++ i )
		{
			if( rs_strnicmp( name, _STD13_MAINCLS_NAMETABLE[i].p, cb ) )
				continue;
...
inline LPCSTR WINAPI MainClassName( SIZE_T uClass )
{
	return nam::_GET_MAIN_NAME(uClass);
}
inline SIZE_T WINAPI MainClassId( LPCSTR aClassName, SIZE_T cClassName )
{
	if( !cClassName ) return nam::_NAME2MCLS( aClassName );
	return nam::_NAME2MCLS2(aClassName, cClassName);
}
