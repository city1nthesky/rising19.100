...
namespace pstsdk
{
	...
	struct IFile : public RefalbeImp
	{
	  virtual ~IFile(){};
      virtual size_t read(std::vector<byte>& buffer, ulonglong offset) const = 0;
	  virtual size_t write(const std::vector<byte>& buffer, ulonglong offset) = 0;
	};
...
//! \ingroup util
class file : public IFile,private boost::noncopyable
{
public:
...
//! and that logic is centralized here.
//! \ingroup util
class RxFile : public IFile , private boost::noncopyable
{
public:
	RxFile(IRXStream* pstm)
	{
		if (pstm == NULL) throw std::runtime_error("fopen failed");
		m_pstm = pstm;
	};
	size_t read(std::vector<byte>& buffer, ulonglong offset) const
	{
        RASSERT(m_pstm->Seek64(offset , SPBegin) == S_OK , 0);

		DWORD dw = (buffer.size()&0xFFFFFFFF);
		if (FAILED(m_pstm->Read(&buffer[0] , dw , NULL))) return 0;
		else return dw;
	...
	size_t write(const std::vector<byte>& buffer, ulonglong offset)
	{
		...
		DWORD dw = (buffer.size()&0xFFFFFFFF);
		if (FAILED(m_pstm->Write(&buffer[0] , dw , NULL))) return 0;
		else return dw;
	}
private:
	UTIL::com_ptr<IRXStream> m_pstm;
