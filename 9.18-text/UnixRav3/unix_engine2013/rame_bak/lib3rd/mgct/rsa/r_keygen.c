...
int RSAFilter(NN_DIGIT *, unsigned int, NN_DIGIT *, unsigned int);
...
R_RANDOM_STRUCT *randomStruct )  /* random structure */
{
	...
	publicKey->bits = privateKey->bits = (short int)(protoKey->bits);
...
int RSAFilter( NN_DIGIT *a, unsigned int aDigits, NN_DIGIT *b, unsigned int bDigits )
{
	int status = 0;
