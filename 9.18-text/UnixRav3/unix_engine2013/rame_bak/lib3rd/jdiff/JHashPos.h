...
public:
	...
	/* Hastable insert */
	void add (hkey akCurHsh, offset_t azPos, int aiEqlCnt ) ;

	/* Hashtable lookup */
	bool get (const hkey akCurHsh, offset_t &azPos) ;

	/* Hashtable printout */
	void print() ;

	/* Printout hashtable distribution */
	void dist(offset_t azMax, int aiBck);
...
private:
	...
	offset_t *mzHshTblPos ;    /* Hash values: positions within the original file       */
