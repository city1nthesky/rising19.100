...
public:
     ...
     */
    JDiff(JFileIn * const apFilOrg, JFileIn * const apFilNew, JOut * const apOut,
...
private:
	/* Context */
	JFileIn * const mpFilOrg ;    // Original file to read
	JFileIn * const mpFilNew ;    // New file to read
	...
	const offset_t mbSrcBkt;     /* Allow bactrace on original file? */
	const offset_t miMchMax;     /* Max number of matches to find */
	const offset_t miMchMin;     /* Min number oif matches to find */
	const offset_t miAhdMax ;    /* Max number of bytes to look ahead */
    const bool mbCmpAll ;   /* Compare all matches, even if data not in buffer? */
    int  miSrcScn;          /* Prescan original file: 0=no, 1=yes, 2=done */

    /* State */
	offset_t mzAhdOrg;     // Current ahead position on original file
	offset_t mzAhdNew;     // Current ahead position on new file
	 ...
	 */
	void ufPutEql(const offset_t &lzPosOrg, const offset_t &lzPosNew, offset_t &lzEql, bool &lbEql) ;
	 ...
	 */
	int ufFndAhd (
	  offset_t const &azRedOrg,        /* read position in original file                     */
	  offset_t const &azRedNew,        /* read position in new file                          */
	  offset_t &azSkpOrg,              /* number of bytes to skip (delete) in original file  */
	  offset_t &azSkpNew,              /* number of bytes to skip (insert) in new file       */
	  offset_t &azAhd                  /* number of bytes to go before similarity is reached */
	);
    ...
    void ufFndAhdGet(JFileIn *apFil, const offset_t &azPos, int &aiVal, int &aiEql, int aiSft) ;
