   ...
   miHshRlb(48), miLodCnt(0), miHshHit(0)
{
	...
	miHshSze = miHshPme * (sizeof(offset_t) + sizeof(hkey));
	mzHshTblPos = (offset_t *) malloc(miHshSze) ;
	...
	if (JDebug::gbDbg[DBGHSH])
		fprintf(JDebug::stddbg, "Hash Ini sizeof=%2d+%2d=%2d, %d samples, %d bytes, address=%p-%p,%p-%p.\n",
				sizeof(hkey), sizeof(offset_t), sizeof(hkey) + sizeof(offset_t),
				miHshPme, miHshSze,
 ...
 */
void JHashPos::add (hkey akCurHsh, offset_t azPos, int aiEqlCnt ){
 ...
 */
bool JHashPos::get (const hkey akCurHsh, offset_t &azPos)
{ int   liIdx ;
 ...
 */
void JHashPos::dist(offset_t azMax, int aiBck){
    int liHshIdx;
    ...
    if (liBckCnt != null){
        ...
        for (liIdx = 0; liIdx < aiBck; liIdx ++)  {
        	...
        	fprintf(JDebug::stddbg, "Hash Dist %8d Pos=" P8zd ":" P8zd " Cnt=%8d Rlb=%d\n",
        			liIdx, (offset_t) liIdx * liHshDiv, (offset_t) (liIdx + 1) * liHshDiv, liBckCnt[liIdx],
