...
public:
	...
	JMatchTable(JHashPos const * cpHsh,  JFileIn  * apFilOrg, JFileIn  * apFilNew, const bool abCmpAll = true);
	 ...
	 * ---------------------------------------------------------------------------*/
	int add (
	  offset_t const &czFndOrgAdd,      /* match to add               */
	  offset_t const &czFndNewAdd,
	  offset_t const &czBseNew,
	  int   ciEqlNew
	);
	 ...
	 * ---------------------------------------------------------------------------*/
	bool get (
	  offset_t const &azBseOrg,       /* base positions       */
	  offset_t const &azBseNew,
	  offset_t &azBstOrg,             /* best position found  */
	  offset_t &azBstNew
	) const;
	 ...
	 * ---------------------------------------------------------------------------*/
	bool cleanup ( offset_t const &czBseNew ) ;
...
private:
	 ...
	 * ---------------------------------------------------------------------------*/
	int check (
	    offset_t &rzPosOrg, offset_t &rzPosNew,
	    int aiLen, int aiSft
	    ) const ;
	...
	typedef struct tMch {
	    ...
	    offset_t izBeg ;       // first found match (new file position)
	    offset_t izNew ;       // last  found match (new file position)
	    offset_t izOrg ;       // last  found match (org file position)
	    offset_t izDlt ;       // delta: izOrg = izNew + izDlt
    ...
    offset_t mzGldDlt ;            /* last gliding match next delta */
