...
inline int inline_LzmaDecode_cb(CLzmaProperties *probs, CProb *prob,
    ...
    unsigned char *outStream, SizeT outSize, SizeT *outSizeProcessed)
{
  ...
  *outSizeProcessed = 0;
  ...
  {
    UInt32 i;
    UInt32 numProbs = Literal + ((UInt32)LZMA_LIT_SIZE << (lc + probs->lp));
			...
			unsigned char *outStream, SizeT outSize, SizeT *outSizeProcessed)
{
	...
	try
	{
		int ret  = inline_LzmaDecode_cb(probs, prob, InCallback, outStream, outSize, outSizeProcessed);
		free(prob);
		return ret;
	}
	catch(...)
