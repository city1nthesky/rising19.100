...
SysReinforceGroup::SysReinforceGroup(QWidget *parent):QWidget(parent)
{
    ...
    m_plScanEvent = new QLabel(u8"上网防护", this);
...
//    m_pcProtectType = new QComboBox(this);
    ...
    lheader << u8"序号" << u8"拦截时间" << u8"处理结果" << u8"防护类型" << u8"访问目标";
...
//setLayout(vlayout);
void SysReinforceGroup::InitControls()
{
    QStringList tlist;
...
//u8"序号" << u8"拦截时间" << u8"处理结果" << u8"防护类型" << u8"访问目标";
void SysReinforceGroup::AddItem(NetAccessEntry &info)
{
    ...
    QTableWidgetItem *itemResult = new QTableWidgetItem(QString(u8"拦截"));
    m_pTable->setItem(rowCount, 2, itemResult);

    QTableWidgetItem *itemType = new QTableWidgetItem(QString("%s").arg(u8"网络防护"));
    m_pTable->setItem(rowCount, 3, itemType);

    QTableWidgetItem  *itemUrl = new QTableWidgetItem(info.mUrl.c_str());
    m_pTable->setItem(rowCount, 4, itemUrl);
}

void SysReinforceGroup::ResetTable()
{
...
void SysReinforceGroup::InitData()
{
    ...
    if(count > 0){
        ...
        while(it != logs.end()){
            ...
            //printf("SysReinforceGroup::InitData before AddItem\n");
...
void SysReinforceGroup::resizeEvent(QResizeEvent *event)
{
