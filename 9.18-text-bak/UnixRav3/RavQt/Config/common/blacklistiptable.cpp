...
void IpIconDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex & index ) const
{
    ...
    :QTableView(parent)
{
    ...
    m_pDelegate = new IpIconDelegate;
...
void BlacklistIPTable::AddIpNameStart()
{
    ...
    if(index.isValid()){
        ...
        if(editWidget != NULL){
            ...
            m_AllItems.push_back(pitem);
            connect(editWidget,SIGNAL(editingFinished()), this, SLOT(AddIpNameEnd()));
        }
    }
}

void BlacklistIPTable::AddIpNameEnd()
{
...
int BlacklistIPTable::AddItem(QString )
{
//    int row_count = m_pModel->rowCount();
//    int rows = m_AllItems.size();
