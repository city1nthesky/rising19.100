...
inline 
static void decrypt(u32 K[40], u32 S[4][256], BYTE PT[16])
{
    u32 T0, T1;
    u32 R0, R1, R2, R3;
...
static void keySched(BYTE M[], int N, u32 **S, u32 K[40], int *k)
{
    u32 Mo[4], Me[4];
    int i, j;
    BYTE vector[8];
...
void CTwoFish::InitKey(unsigned char *key, int bits)
{
	UINT32 *S;
	int k;
	keySched(key, bits, &S, K, &k);
	fullKey(S, k, QF);
	free(S);
}
void CTwoFish::SetExpandKey(UINT32 AK[40], UINT32 AQF[4][256])
{
	{
		memcpy(K, AK, sizeof(K));
		memcpy(QF, AQF, sizeof(QF));
	}
}

void CTwoFish::ECB_Encrypt(UINT8 PT[16])
{
	encrypt(K, QF, PT);
}

void CTwoFish::ECB_Decrypt(UINT8 PT[16])
{
	decrypt(K, QF, PT);
}
...
void bench()
{
	...
	_ftime(&tb);
	tv_start.tv_sec = tb.time;
	tv_start.tv_usec = tb.millitm;
	...
	_ftime(&tb);
	tv_end.tv_sec = tb.time;
	tv_end.tv_usec = tb.millitm;
